...
Since there are no Github-hosted runners, the `warm` job cannot be dependent on a self-hosted runner. The cron job is set on <a href='https://console.cron-job.org/login'>cron-job.com</a> and runs every 30mins to keep the Render instance warm. 
...
`cd ./actions-runner && ./run.sh` 75.2.60.5 
